...
class StabilizationPub(Node):
    def __init__(self):
        ...
        # Subscribers
        self.depthSub = self.create_subscription(Float32, 'bar02/depth', self.depth_callback, 10)
        self.toggleSub = self.create_subscription(
            Bool, 'stabilization_toggle', self.toggle_callback, 10
        )
        ...
        self.twistSub = self.create_subscription(
            Twist, 'twist', self.manual_input_callback, twist_qos
        )
    ...
    def toggle_callback(self, msg: Bool):
        ...
        if self.depth_hold_enabled:
            ...
            self.get_logger().info(
                f'Depth hold ENABLED. Target depth set to {self.target_depth:.2f} m'
            )
        else:
